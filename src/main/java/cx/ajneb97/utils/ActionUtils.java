...
public class ActionUtils {
    ...
    public static void executeAction(Player player, String actionText, Codex plugin, ArrayList<CommonVariable> variables){
        ...
        switch(actionType){
            ...
            case "player_command":
                ...
                break;
            case "playsound":
                ActionUtils.playSound(player,actionLine,plugin);
                break;
            case "title":
    ...
    public static void playSound(Player player, String soundLine, Codex plugin){
        ...
        try {
            ...
            String msg = plugin.getMessagesConfig().getString("soundInvalid").replace("%sound%", sep[0]);
            Bukkit.getConsoleSender().sendMessage(Codex.prefix + MessagesManager.getColoredMessage(msg));
            return;
        }
    ...
    public static void message(Player player,String actionLine){
        player.sendMessage(MessagesManager.getCustomFishingColoredMessage(actionLine));
    }

    public static void centeredMessage(Player player,String actionLine){
        actionLine = MessagesManager.getCustomFishingColoredMessage(actionLine);
