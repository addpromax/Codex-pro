...
public class VerifyManager {
    ...
    public void sendVerification(Player player) {
        ...
        if(errors.isEmpty()) {
            player.sendMessage(MessagesManager.getColoredMessage(plugin.getMessagesConfig().getString("verifyNoError")));
        }else {
    ...
    public void verify() {
        ...
        if(inventoryManager.getInventory("main_inventory") == null){
            errors.add(new CodexInventoryDefaultNotExistsError(plugin, "inventory.yml",null,true,"main_inventory"));
            criticalErrors = true;
        }
    }
        ...
        for(CommonItem item : allItems){
            if(item != null){
                if(!verifyItem(item.getId())){
                    errors.add(new CodexInvalidItem(plugin, category.getName()+".yml",null,true,item.getId()));
                    criticalErrors = true;
                }
            }
        }
    ...
    public void verifyActions(List<String> actions,String actionGroup,String fileName){
        ...
        for(int i=0;i<actions.size();i++){
            ...
            errors.add(new CodexActionError(plugin, fileName, actions.get(i), false, actionGroup, (i+1)+""));
        }
    }
        ...
        for(CommonInventoryItem item : items){
           ...
           if(type != null){
               if(type.startsWith("category: ")){
                   ...
                   if(categoryManager.getCategory(categoryName) == null){
                       errors.add(new CodexInventoryInvalidCategoryError(plugin, "inventory.yml",null,true,categoryName,
                   ...
                   if(category == null || category.getDiscovery(discoveryName) == null){
                       errors.add(new CodexInventoryInvalidDiscoveryError(plugin, "inventory.yml",null,true,discoveryName,categoryName,
                               inventory.getName(),item.getSlotsString()));
                       criticalErrors = true;
                   }
               }
           ...
           if(openInventory != null){
               if(inventoryManager.getInventory(openInventory) == null){
                   errors.add(new CodexInventoryNotExistsError(plugin, "inventory.yml", null, true, openInventory));
                   criticalErrors = true;
               }
           }
           ...
           if(commonItem != null){
               if(!verifyItem(commonItem.getId())){
                   errors.add(new CodexInvalidItem(plugin, "inventory.yml",null,true,commonItem.getId()));
                   criticalErrors = true;
               }
           }
           ...
           for(int slot : item.getSlots()){
               if(slot >= maxSlots){
                   errors.add(new CodexInventoryInvalidSlotError(plugin, "inventory.yml",null,true,slot,
